...
620c17e844a76c9257fba0c1f1df1f50f617381c6e79acec5c0ad2960e58cf72  recovery-no-credential.patch
...
9a5556aac763b195b4369aeaa3084924343afff60020c310402b9d060f789cab  recovery-data-lifecycle.patch
...
layout. All ten patches apply cleanly in their documented order with
`git apply --check --whitespace=error-all` to their pinned commits.

## Responsibilities

- `recovery.patch`, `recovery-no-credential.patch`,
  `recovery-password-probe.patch`, `recovery-setup-de-ce-guard.patch`,
  `recovery-fast-startup.patch`, and `recovery-data-lifecycle.patch` add
  the H.40 adapter,
...
- `stock-first/build_full_boot.sh` starts from the checked H.40 ramdisk, builds
  ...
  manifests, verifies complete CPIO parent-directory closure, restores the H.40
  boot-v2 framing, and verifies the final AVB-padded `boot.img` independently.
